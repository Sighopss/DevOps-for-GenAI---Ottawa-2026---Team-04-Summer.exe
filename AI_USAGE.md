...
Verifiable from this repo without taking anyone's word for it: every feature branch carries a matching `handoffs/<name>-<id>-<slug>.md`, committed on that branch and repeated as the PR body. If a change has no handoff, it did not go through this process. Eleven PRs are merged to `main` as of this commit, so `git log --merges` and the files under `handoffs/` are the record — Michael's `web/` lane is the only one with no handoff, because it has not started.
