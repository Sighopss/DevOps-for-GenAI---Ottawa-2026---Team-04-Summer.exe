...
1. **WAF is now attached to CloudFront, but evaluation against live traffic is not yet confirmed (updated 2026-08-22, ~19:16 UTC).** The original finding — WAFv2 cannot attach to an HTTP API, so `aws_wafv2_web_acl_association` failed on every apply — was fixed in #100/#128 by replacing the ACL with a `CLOUDFRONT`-scoped one and setting `web_acl_id` directly on the distribution (`infra/cloudfront.tf`); `aws wafv2 list-web-acls --scope CLOUDFRONT` and the distribution's `WebACLId` both confirm the attachment. **Not yet true:** ~13 minutes after apply, a SQLi and an XSS payload sent to the edge both still returned `200`, and `AWS/WAFV2` `AllowedRequests`/`BlockedRequests` have zero datapoints — consistent with normal CloudFront edge propagation (10–20 min) but not yet observed working. Re-check `AllowedRequests`/`BlockedRequests` before claiming the ACL filters anything. **Impact until then:** abuse cases that assumed WAF (rate/flood) still rest entirely on the in-Lambda caps (batch ≤100, 1 MB body, 10k-char field, depth-32) + API Gateway default throttling.
...
- No per-tenant ingest rate limit — WAF is now attached to CloudFront but not yet confirmed evaluating traffic (see above), so flood protection today is still bounded by in-Lambda caps + gateway throttling + 7-day TTL. Production: usage plans / per-key throttling.
- Explorer UI is now published — CloudFront `/` returns 200 (welcome) and `/explorer.html` serves the live flight explorer (re-checked 2026-08-22, after this file's original write-up). Direct navigation to `/explorer` without the extension still falls back to the welcome page — see `README.md` Demo integrity.
