...
- Stub/live (P-15): The **Public URL** line in `README.md` is `TBD — filled by Trevor after the first green deploy.yml`. It is deliberately not a hostname, because no deploy has run. The README's own demo-integrity table declares Explorer Day 1 as fixtures, `TRACEVAULT_FAKE_BEDROCK=1` as a faked model call, and an unset ingest URL as `sdk/.last-flight.json` only. The exact Bedrock model id in `AI_USAGE.md` is likewise `TBD` until the deployed value is fixed.
...
- contract / URL / header / path:
  ...
  - Michael: same, for `web/`, `PRODUCT.md`, `DESIGN.md`, and Playwright. The README's Demo integrity table asserts Explorer Day 1 renders `contracts/fixtures/` and Day 2 hits `GET /v1/traces*` — if your Day 2 cutover slips, tell Trevor so that table stays true.
