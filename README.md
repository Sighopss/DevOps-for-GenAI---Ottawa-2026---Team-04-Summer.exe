...
| Bedrock model call | Yes | Yes — `demo-app/` on `main` | **Apply + model access** | Live path uses Nova Lite (default) or Claude 3.5 Sonnet + Titan Embed V2 — all three on the IAM allowlist. A real Bedrock call **unless** `TRACEVAULT_FAKE_BEDROCK=1`. Every test to date has used the fake. If the demo is run that way, we say so out loud. Account must enable those three models in Bedrock and the next apply must land the updated OIDC policy. |
